<template>
    <div style="width:100%;height:100%;" ref="maxContainer">
      ...
      <Modal v-model="ftpModal" :width="720" :title="modalTitle" :mask-closable="false" class-name="vertical-center-modal">
        <Form ref="ftpInfoFrom" :model="ftpinfo" label-position="top">
            ...
            <Row>
              ...
              <Col span="12" class="modal-form-item-padding">
                <FormItem label="根目录" prop="RemotePath" :rules="{ required: true, validator: validateFtpPath,trigger: 'blur' }">
...
export default {
  data () {
    return {
      ...
      validateFtpPath: function (rule, value, callback) {
        if (!value) {
          callback(new Error('根目录不能为空!'))
        } else if (/^[a-zA-Z]:(((\\(?! )[^/:*?<>""|\\]+)+\\?)|(\\)?)\s*$/.test(value)) {
          callback()
        } else {
          callback(new Error('根目录格式错误!'))
        }
      },
      tableH: 0
    }
